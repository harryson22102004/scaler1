...
class Scenario:
    ...
    services: Dict[str, Dict[str, Any]] = field(default_factory=lambda: cast(Dict[str, Dict[str, Any]], {}))
    file_overrides: Dict[str, Dict[str, Any]] = field(default_factory=lambda: cast(Dict[str, Dict[str, Any]], {}))
    dir_additions: List[str] = field(default_factory=lambda: cast(List[str], []))
    faults: List[Fault] = field(default_factory=lambda: cast(List[Fault], []))
    cascades: List[CascadeRule] = field(default_factory=lambda: cast(List[CascadeRule], []))
    objectives: List[Objective] = field(default_factory=lambda: cast(List[Objective], []))
    max_steps: int = 50
    hints: List[str] = field(default_factory=lambda: cast(List[str], []))
...
class ScenarioGrader:
    ...
    def evaluate(self, objectives: List[Objective]) -> Tuple[float, Dict[str, Any]]:
        total = 0.0
...
def _scenario_log_analysis() -> Scenario:
    return Scenario(
        ...
        difficulty="easy",
        objectives=[
            Objective("Check the current working directory",
                      "command_was_run", {"pattern": r"pwd"},
                      points=0.01),
            Objective("List files in /var/log",
                      "command_was_run", {"pattern": r"ls.*/var/log"},
                      points=0.04),
...
def _scenario_permission_repair() -> Scenario:
    return Scenario(
        ...
        objectives=[
            Objective("Locate the cleanup script",
                      "command_was_run", {"pattern": r"ls.*/home/user/scripts/cleanup\.sh"},
                      points=0.01),
            Objective("Check script permissions",
                      "command_was_run", {"pattern": r"ls -l.*/home/user/scripts/cleanup\.sh"},
                      points=0.04),
...
def _scenario_disk_space_crisis() -> Scenario:
    return Scenario(
        ...
        objectives=[
            Objective("Check disk usage with df",
                      "command_was_run", {"pattern": r"df"},
                      points=0.1),
            Objective("Find large files in /var/log",
                      "command_was_run", {"pattern": r"du|ls -l"},
                      points=0.2),
